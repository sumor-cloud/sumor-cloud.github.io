...
API Middleware est un intergiciel pour Node.JS.
...
type supporte seulement `number`, `string`, `boolean`, `array`, `object`
...
Pour supporter les fonctions JS dans le fichier de configuration, vous pouvez utiliser le fichier `config.js`, remplacez `plus.json` par `plus.config.js`
...
- `mime` type MIME du fichier téléchargé (par ex. image/png)
- `encoding` encodage du fichier téléchargé (par ex. 7bit)
