...
yaml 파일을 사용하여 구성 파일을 정의할 수 있으며, `plus.json`을 `plus.yml`로 바꿔 사용합니다.

유형은 `number`, `string`, `boolean`, `array`, `object`만 지원됩니다.
...
구성 파일에서 js 함수를 지원하려면 `config.js` 파일을 사용하고, `plus.json`을 `plus.config.js`로 바꿔 사용합니다.
