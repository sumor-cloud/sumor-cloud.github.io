...
다양한 수준으로 로그를 출력하고, scope, id 및 시간대를 사용자 정의할 수 있습니다.
...
일부 경우에는 로그를 분류해야 할 때 `scope`가 사용됩니다.
...
일부 경우에는 사용자 식별자가 필요할 때 `id`가 사용됩니다.
...
대부분의 경우에는 특정 수준의 로그만 출력할 필요가 있습니다. 그럼 저장하고 표시할지 여부를 결정할 수 있습니다.
...
logger.fatal('Hello World!') // fatal은 가장 높은 수준이므로 심각한 오류만 출력됩니다.
```
...
const code = {
  trace: {
    HTTP_ACCESS: '사용자가 HTTP를 통해 접속하고 IP 주소는 {ip}'
  },
  debug: {
    USER_TOKEN_LOADED: '사용자 로그인 정보가 읽히고 사용자 ID는 {id}'
  },
  info: {
